...
export default function request(url) {
    ...
    return fetch(newUrl, newOptions).then(checkStatus).then(function (response) {
        ...
        if (status === 403) {
            router.push('/exception/403'); // eslint-disable-next-line consistent-return
            return;
        }
        if (status <= 504 && status >= 500) {
            router.push('/exception/500'); // eslint-disable-next-line consistent-return
            return;
        }
